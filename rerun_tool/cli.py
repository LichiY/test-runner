...
def process_entry(entry: TestEntry, workspace_dir: str, rerun_count: int, mode: RerunMode, docker_mode: str, build_timeout: int, test_timeout: int, build_retries: int, runner_backend: RunnerBackend = RunnerBackend.STANDARD) -> TestRunResult:  # 保留旧版 process_entry 兼容包装器。 
    request = request_from_test_entry(entry, runner_backend=runner_backend)  # 先把旧版条目转换成统一运行请求。 
    config = ExecutionConfig(rerun_count=rerun_count, mode=mode, docker_mode=docker_mode, build_timeout=build_timeout, test_timeout=test_timeout, build_retries=build_retries)  # 构造统一执行配置。 
...
def _run_requests(requests: List[object], args: argparse.Namespace, mode: RerunMode) -> None:  # 统一执行一组已经加载完成的运行请求。 
    ...
    config = ExecutionConfig(rerun_count=args.rerun, mode=mode, docker_mode=args.docker_mode, build_timeout=args.build_timeout, test_timeout=args.test_timeout, build_retries=args.build_retries)  # 构造统一执行配置。 
    ...
    if args.resume:  # 只有显式开启 resume 时才读取历史结果文件。 
        ...
        if restored_results:  # 只有恢复到历史结果时才跳过对应请求。 
            restored_keys = {_result_request_key(result) for result in restored_results}  # 提取所有已恢复结果的稳定请求键。 
            active_requests = [request for request in active_requests if request.request_key not in restored_keys]  # 只保留尚未处理的新请求。 
            all_results.extend(restored_results)  # 将历史结果放入总结果列表中。 
            logger.info(f"Resume enabled: loaded {len(restored_results)} existing results and skipped them")  # 记录本次恢复命中数量。 
    ...
    for i, request in enumerate(active_requests):  # 逐条处理当前请求列表。 
        logger.info(f"\n[{i + 1}/{len(active_requests)}] Processing entry {request.index}")  # 记录当前进度与目标索引。 
...
def _add_shared_cli_arguments(parser: argparse.ArgumentParser) -> None:  # 为不同命令统一追加共享参数。 
    ...
    parser.add_argument('--resume', action='store_true', help='Resume from an existing output CSV by skipping already processed requests')  # 添加断点续跑参数。 
