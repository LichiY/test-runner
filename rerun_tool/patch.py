...
def _infer_reference_import_lines(test_file: str, *code_fragments: str) -> Tuple[str, ...]:  # 根据参考补丁正文、当前测试文件和原始方法签名共同推断高频第三方 import 与 static import。
    ...
    for symbol, import_line in REFERENCE_CODE_IMPORT_INFERENCE.items():  # 逐个检查常见第三方类型符号。
        if re.search(rf'\b{re.escape(symbol)}\b', code):  # 只有代码正文明确出现该符号时才推断 import。
            inferred_imports.append(_resolve_reference_symbol_import_line(test_file, symbol, import_line))  # 优先复用当前仓库和当前文件已有线索，避免盲目绑到错误库。
    for symbol in ('Sets', 'Lists', 'Maps', 'Config', 'Option'):  # 这几个符号歧义高，必须走带上下文的解析逻辑。
        if re.search(rf'\b{re.escape(symbol)}\b', code):  # 只有正文真的用到了当前符号时才继续解析。
            resolved_import = _resolve_reference_symbol_import_line(test_file, symbol, '')  # 先让已有仓库线索和当前文件上下文决定导入路径。
...
def _collect_reference_dependency_snippets(candidate: ReferencePatchCandidate, entry: Optional[TestEntry] = None, import_lines: Tuple[str, ...] = ()) -> List[str]:  # 汇总参考补丁显式给出的 dependency 片段与可从候选代码和原始方法签名推断出的测试依赖。
...
def apply_reference_patch_context(repo_dir: str, entry: TestEntry, test_file: str, candidate: ReferencePatchCandidate) -> Tuple[bool, str]:  # 将“已知成功代码片段里显式或可推断的上下文”同步应用到工作区。
    ...
    dependency_snippets = _collect_reference_dependency_snippets(candidate, entry, tuple(import_lines))  # 汇总显式与推断出的 dependency 片段。
...
def backport_fixed_sha_test_helpers(repo_dir: str, entry: TestEntry, test_file: str, build_output: str, git_timeout: int = 300, git_retries: int = 1) -> Tuple[bool, str]:  # 当 generated_patch 依赖 fixed_sha 才引入的测试 helper、field 或 import 时，把最小必需上下文迁回 original_sha 工作区。
    missing_methods = _extract_missing_methods(build_output, test_file)  # 只处理当前目标测试文件里缺失的方法 helper。
    ...
    if not missing_methods and not missing_variables and not missing_classes:  # 没有任何可从 fixed_sha 学习的上下文时无需继续。
    ...
    try:  # 读取当前目标测试文件内容，供后续判断是否真的缺少 helper 定义。
        with open(test_file, 'r', encoding='utf-8') as f:  # 打开当前 original_sha 工作区里的测试文件。
            current_content = f.read()  # 保存当前文件内容，后续会在其上追加 helper。
    except Exception as e:  # 读取失败时直接返回明确错误。
        return False, f'Failed to read test file for fixed_sha helper backport: {e}'
    if not backup_file(test_file):  # 在追加 helper 前确保已经保存原始基线。
        return False, f"Failed to backup test file before fixed_sha helper backport: {os.path.basename(test_file)}"
    target_rel_path = os.path.relpath(test_file, repo_dir).replace(os.sep, '/')  # 计算目标测试文件相对仓库根目录的路径，便于去 fixed_sha 中读取同路径文件。
    backported_candidates: List[ReferencePatchCandidate] = []  # 收集这次真正迁回来的 helper、field 和 import 候选，上下文注入阶段会继续复用。
    backport_messages = []  # 收集每个回补动作的来源摘要，最终写回错误消息头部。
    for method_name in missing_methods:  # 顺序处理当前编译错误里暴露出的每个缺失 helper。
        if _file_contains_method_definition(current_content, method_name):  # 当前工作区里如果已经有了这个 helper，就不重复追加。
            continue  # 跳过已满足的方法定义。
        helper_candidate = _find_fixed_sha_helper_candidate(repo_dir, entry, fixed_sha, target_rel_path, method_name, git_timeout)  # 去 fixed_sha 中寻找最可信的 helper 定义来源。
        if helper_candidate is None:  # fixed_sha 里也没找到时，把机会留给后续真正的失败诊断。
            continue  # 当前 helper 无法回溯，继续处理下一个。
        current_content, inserted = _append_helper_method_to_file(current_content, helper_candidate.test_code, method_name)  # 把 helper 定义直接追加回当前测试类末尾。
        if not inserted:  # 追加失败或当前方法已经存在时跳过。
            continue  # 不把这个候选计入真正的 backport。
        backported_candidates.append(helper_candidate)  # 记录这次真的迁回来的 helper 候选。
        backport_messages.append(f"{method_name}<-{helper_candidate.source_path}")  # 保存方法名与来源路径，便于最终诊断。
    for variable_name in missing_variables:  # 顺序处理当前目标测试文件缺失的字段或常量。
        ...
        if not inserted:  # 追加失败或当前字段已经存在时跳过。
            continue  # 不把这个候选计入真正的 backport。
        backported_candidates.append(field_candidate)  # 记录这次真的迁回来的字段候选。
    ...
    if import_candidate is not None:  # 只有 fixed_sha 同文件明确给出了所需导入时才继续。
        backported_candidates.append(import_candidate)  # 记录这次真的迁回来的 import 候选。
    ...
    with open(test_file, 'w', encoding='utf-8') as f:  # 将追加 helper 后的测试文件落回工作区。
        f.write(current_content)  # 写入新的测试文件内容。
    context_messages = []  # 收集 helper 自身需要的 import 和 pom 上下文补齐记录。
    for helper_candidate in backported_candidates:  # 顺序把每个 helper、field 或 import 候选依赖的 import 和 dependency 一起补回当前工作区。
        context_ok, context_msg = apply_reference_patch_context(repo_dir, entry, test_file, helper_candidate)  # 复用统一的 import/pom 注入逻辑，不引入新的“替代补丁”语义。
...
def _find_fixed_sha_helper_candidate(repo_dir: str, entry: TestEntry, fixed_sha: str, target_rel_path: str, method_name: str, git_timeout: int) -> Optional[ReferencePatchCandidate]:  # 按“同文件 -> 同目录 -> 同模块测试树”的顺序在 fixed_sha 里找最可信的 helper 定义。
    ...
    for relative_path in _iter_fixed_sha_helper_candidate_paths(repo_dir, entry, fixed_sha, target_rel_path, git_timeout):  # 顺序遍历优先级已排好的 fixed_sha 候选文件。
        ...
        if not helper_method:  # 没有找到可安全迁回的 helper 定义时继续试下一个文件。
            continue  # 跳到下一候选。
        return _build_fixed_sha_member_candidate(f'fixed_sha:{normalized_path}', file_content, helper_method)  # 返回当前最可信的 fixed_sha helper 候选，并只保留 snippet 真正需要的 import。
...
def _find_fixed_sha_field_candidate(repo_dir: str, entry: TestEntry, fixed_sha: str, target_rel_path: str, field_name: str, git_timeout: int) -> Optional[ReferencePatchCandidate]:  # 按“同文件 -> 同目录 -> 同模块测试树”的顺序在 fixed_sha 里找最可信的字段或常量定义。
    ...
    for relative_path in _iter_fixed_sha_helper_candidate_paths(repo_dir, entry, fixed_sha, target_rel_path, git_timeout):  # 复用与 helper 相同的固定搜索顺序。
        ...
        if not field_definition:  # 没有找到可安全迁回的字段定义时继续试下一个文件。
            continue  # 跳到下一候选。
        return _build_fixed_sha_member_candidate(f'fixed_sha:{normalized_path}', file_content, field_definition)  # 返回当前最可信的 fixed_sha 字段候选，并只保留 snippet 真正需要的 import。
...
def _find_fixed_sha_import_candidate(repo_dir: str, fixed_sha: str, target_rel_path: str, missing_classes: List[str], git_timeout: int) -> Optional[ReferencePatchCandidate]:  # 从 fixed_sha 同文件里提取当前目标测试真正缺失的类导入。
    ...
    return ReferencePatchCandidate(source_path=f'fixed_sha:{target_rel_path}', test_code='', imports=import_lines)  # 返回一个仅承载 import 的候选，后续复用统一上下文注入逻辑。


def _build_fixed_sha_member_candidate(source_path: str, file_content: str, member_code: str) -> ReferencePatchCandidate:  # 根据 fixed_sha 源文件和待迁回成员构造只保留最小上下文的候选。
    ...
    return ReferencePatchCandidate(source_path=source_path, test_code=member_code, imports=import_lines)  # 返回裁剪过 import 的成员候选。
...
def _iter_fixed_sha_helper_candidate_paths(repo_dir: str, entry: TestEntry, fixed_sha: str, target_rel_path: str, git_timeout: int):  # 生成 fixed_sha helper 检索时的候选文件顺序。
    yield target_rel_path  # 永远优先看目标测试文件本身，很多 helper 就是同类新增的私有方法。
    target_dir = os.path.dirname(target_rel_path).replace(os.sep, '/')  # 读取目标测试文件所在目录，作为“同包其他文件”的候选前缀。
    module_prefix = '' if not getattr(entry, 'module', '') or entry.module == '.' else entry.module.strip('/').replace(os.sep, '/')  # 统一规整模块前缀。
    search_prefixes = [target_dir]  # 先从目标测试文件所在目录开始扩展搜索。
    for test_dir in FIXED_SHA_HELPER_SEARCH_DIRS:  # 再把同模块测试树加入候选前缀序列。
        prefix = '/'.join(part for part in [module_prefix, test_dir] if part)  # 只拼接非空片段，避免根模块场景出现多余斜杠。
        if prefix and prefix not in search_prefixes:  # 保持顺序的同时避免重复前缀。
            search_prefixes.append(prefix)  # 记录当前测试树前缀。
    for prefix in search_prefixes:  # 按优先级顺序读取每个前缀下的文件。
        files_ok, files_or_message = list_files_at_revision(repo_dir, fixed_sha, prefix, timeout=git_timeout)  # 列出 fixed_sha 当前前缀下的全部文件。
        if not files_ok:  # 某个前缀读取失败时直接跳过，让其他前缀继续。
            continue  # 当前前缀无法读取不应阻断整个 helper 搜索。
        for relative_path in files_or_message:  # 顺序遍历当前前缀下的文件。
            if not relative_path.endswith('.java'):  # 当前 helper 回退只处理 Java 测试源码。
                continue  # 过滤掉无关文件。
            yield relative_path  # 把当前候选文件路径交给上层逐个解析。
...
def _find_top_level_class_closing_brace(content: str) -> int:  # 找到顶层类真正的闭合大括号位置，避免简单 rfind 被内部类或注释误导。
    depth = 0  # 记录当前大括号深度。
    class_depth_started = False  # 记录是否已经进入过至少一层类体。
    index = 0  # 使用原始文本索引逐字符扫描，确保返回的位置仍然可直接用于原文切片。
    while index < len(content):  # 顺序扫描原始源码文本。
        ch = content[index]  # 读取当前字符。
        if ch == '/' and index + 1 < len(content) and content[index + 1] == '*':  # 跳过块注释，避免注释中的大括号干扰计数。
            end = content.find('*/', index + 2)  # 查找块注释结束位置。
            index = len(content) if end == -1 else end + 2  # 没找到结束符时直接视为扫描结束。
            continue  # 进入下一轮扫描。
        if ch == '/' and index + 1 < len(content) and content[index + 1] == '/':  # 跳过行注释，避免注释中的大括号干扰计数。
            end = content.find('\n', index + 2)  # 查找行注释结束位置。
            index = len(content) if end == -1 else end + 1  # 没有换行时直接扫描结束。
            continue  # 进入下一轮扫描。
        if ch == '"':  # 跳过字符串字面量，避免字符串里的大括号干扰计数。
            index = _skip_string(content, index, '"')  # 复用现有字符串跳过逻辑，并保持原始索引。
            continue  # 进入下一轮扫描。
        if ch == "'":  # 跳过字符字面量，避免字符里的大括号干扰计数。
            index = _skip_string(content, index, "'")  # 复用现有字符跳过逻辑，并保持原始索引。
            continue  # 进入下一轮扫描。
        if ch == '{':  # 每遇到一个左大括号就进入更深一层。
            depth += 1  # 更新当前深度。
            class_depth_started = True  # 说明当前文件里至少存在一个类体。
        elif ch == '}':  # 每遇到一个右大括号就退出一层。
            depth -= 1  # 先更新当前深度。
            if class_depth_started and depth == 0:  # 回到 0 时说明刚好走出了顶层类体。
                return index  # 返回顶层类闭合大括号在原始文本中的位置。
        index += 1  # 默认向后移动一个原始字符。
...
def _extract_missing_variables(build_output: str, test_file: str = '') -> List[str]:  # 从编译输出来提取当前目标测试文件里缺失的字段或常量名。
    ...
    for raw_line in build_output.splitlines():  # 逐行分析编译输出。
        ...
        if ('cannot find symbol' in line or '找不到符号' in line):  # 进入新的编译错误块时先判断是否属于目标文件。
            current_error_matches_target = (target_name in line) if target_name else True  # 只有目标测试文件对应的错误块才参与后续变量提取。
...
def _extract_missing_classes(build_output: str, test_file: str = '') -> List[str]:  # 从编译输出来提取当前目标测试文件里缺失的类名。
    ...
    for raw_line in build_output.splitlines():  # 逐行分析编译输出。
        line = raw_line.strip()  # 去掉首尾空白后便于模式匹配。
        if ('cannot find symbol' in line or '找不到符号' in line):  # 进入新的编译错误块时先判断是否属于目标文件。
            current_error_matches_target = (target_name in line) if target_name else True  # 只有目标测试文件对应的错误块才参与后续类名提取。
...
def _resolve_reference_symbol_import_line(test_file: str, symbol: str, fallback_import_line: str) -> str:  # 基于当前测试文件和仓库上下文为参考信号解析最终 import 行。
...
def _insert_import_lines(content: str, import_lines: List[str]) -> str:  # 将新的 import 行插入到 package 和现有 import 之后。
    ...
    for import_line in import_lines:  # 逐个处理传入的 import 行。
        simple_name = _import_simple_name(import_line)  # 读取当前待插入 import 对应的简单名称。
        if simple_name and simple_name in existing_simple_names and existing_simple_names[simple_name] != import_line.strip():  # 当前文件里已经存在同名但不同路径的普通 import 时不能再追加。
