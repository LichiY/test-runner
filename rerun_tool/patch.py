...
JAVA_IMPORT_CANDIDATES = {  # 为常见标准库与集合类型提供保守的 import 推断表。
    ...
    'Assertions': 'org.assertj.core.api.Assertions',  # Assertions 常见于 AssertJ。
    'JSONAssert': 'org.skyscreamer.jsonassert.JSONAssert',  # JSONAssert 来自 jsonassert 库。
    'JSONCompareMode': 'org.skyscreamer.jsonassert.JSONCompareMode',  # JSONCompareMode 来自 jsonassert 库。
    'ObjectMapper': 'com.fasterxml.jackson.databind.ObjectMapper',  # ObjectMapper 常见于 Jackson。
}
...
def _find_existing_import_path(repo_root: str, symbol: str) -> Optional[str]:  # 在仓库源码中搜索唯一的现成 import 语句。
    import_pattern = re.compile(rf'^\s*import\s+([A-Za-z0-9_.]*\.{re.escape(symbol)})\s*;\s*$')  # 匹配完整的 Java import 语句。
    candidate_imports = []  # 保存命中的 import 路径。
            ...
            try:  # 单个文件读取失败时直接跳过。
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:  # 以宽松模式读取 Java 文件。
                    for line in f:  # 按行扫描 import 语句。
                        ...
                        if match:  # 命中目标 import 语句时记录下来。
                            candidate_imports.append(match.group(1))  # 保存完整 import 路径。
    ...
    if len(unique_imports) == 1:  # 只有唯一候选时才安全复用。
        return unique_imports[0]  # 返回仓库中唯一的 import 路径。
    return None  # 多个或零个候选都视为无法安全推断。
