...
def _reference_candidate_sort_key(candidate: ReferencePatchCandidate, entry: TestEntry) -> Tuple[int, float, int, int, int, int, int]:  # 将“来源可信度、结构保持程度、API 兼容性”合并成离线分析用排序键。
...
def _reference_candidate_structure_similarity(candidate: ReferencePatchCandidate, entry: TestEntry) -> float:  # 计算候选与当前 generated_patch 的结构相似度。
    return _reference_candidate_patch_similarity(candidate, entry)  # 当前仅保留这一种结构相似度定义。
