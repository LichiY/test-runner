...
JAVA_STATIC_IMPORT_CANDIDATES = {  # 为常见的静态断言与匹配器方法提供保守的 static import 推断表。
    'atLeastOnce': 'org.mockito.Mockito.atLeastOnce',  # atLeastOnce 是 Mockito verify 语义的稳定静态工厂。
...
FIXED_SHA_HELPER_SEARCH_DIRS = ('src/test/java', 'src/test/groovy', 'src/test/scala', 'test/java', 'test/groovy', 'test/scala', 'test')  # fixed_sha helper 回溯同时兼容 `src/test/*` 与老仓库常见的 `test/...` 布局，避免漏掉可复用 helper。
...
def _infer_reference_import_lines(test_file: str, *code_fragments: str) -> Tuple[str, ...]:  # 根据参考补丁正文、当前测试文件和原始方法签名共同推断高频第三方 import 与 static import。
    ...
    for helper_name, import_line in REFERENCE_CODE_STATIC_IMPORT_INFERENCE.items():  # 再检查高频 helper 的静态导入。
        if _reference_code_uses_static_helper(code, helper_name):  # 仅在代码里真实出现裸 helper 调用时才补对应 static import。
            resolved_static_import = _resolve_missing_method_reference(test_file, helper_name, code) if test_file else None  # `when(...)` 这类 helper 需要结合当前补丁语义区分 Mockito 与 catch-exception。
            if resolved_static_import:  # 当前仓库和代码语义已经能说明真实 helper 所属时优先采用它。
                inferred_imports.append(f'import static {resolved_static_import};')  # 记录当前推断出的 static import。
            elif import_line:  # 其余 helper 继续回退到静态映射表。
    ...
    if re.search(r'(?<![A-Za-z0-9_$.])assertThat\s*\(', code):  # generated_patch 里直接出现裸 `assertThat(...)` 时也要尝试补静态导入。
        import_path = _resolve_missing_method_reference(test_file, 'assertThat', code) if test_file else None  # 继续沿用已有的 AssertJ/Hamcrest 选择逻辑。
...
def _looks_like_mockito_when_usage(code: str) -> bool:  # 区分 `Mockito.when(...).thenReturn(...)` 与 catch-exception 的 `when(service).call()` 语义。
    normalized_code = (code or '').lower()  # 统一转小写，便于识别链式 stub 与 Mockito 线索。
    if not _has_unqualified_helper_call(code, 'when'):  # 没有裸 `when(...)` 时直接返回假。
        return False
    if re.search(r'(?<![a-z0-9_$.])when\s*\([\s\S]{0,240}?\)\s*\.\s*then(?:return|throw|answer|callrealmethod|callrealmethods?)\b', normalized_code):  # `.thenReturn/.thenThrow/...` 是 Mockito stub 的稳定特征。
        return True
    mockito_markers = ('org.mockito', 'mock(', 'spy(', 'verify(', 'argumentcaptor', 'eq(', 'any(')  # 只保留 Mockito 特有或高置信度的 stub 线索，避免把 AssertJ 的 `isNotNull()` 误判成 Mockito 语境。
    return any(marker in normalized_code for marker in mockito_markers)  # 只有在 Mockito 语境明确出现时才进一步判为 Mockito `when(...)`。


def _looks_like_catch_exception_when_usage(code: str) -> bool:  # 区分真正需要 catch-exception 的 `when(...)` 调用。
    normalized_code = code or ''  # 统一规整为空串，便于后续模式判断。
    if 'com.googlecode.catchexception' in normalized_code or _has_unqualified_helper_call(normalized_code, 'caughtException'):  # 显式包名或 `caughtException()` 是 catch-exception 的强信号。
        return True
    if not _has_unqualified_helper_call(normalized_code, 'when'):  # 没有裸 `when(...)` 时无需继续。
        return False
    if _looks_like_mockito_when_usage(normalized_code):  # 明显是 Mockito stub 时绝不应再回退到 catch-exception。
        return False
    return bool(re.search(r'(?<![A-Za-z0-9_$.])when\s*\([\s\S]{0,240}?\)\s*\.\s*[A-Za-z_][A-Za-z0-9_]*\s*\(', normalized_code))  # `when(service).doSomething()` 这类调用更符合 catch-exception 语义。
...
def _signals_require_dependency(dependency_key: str, signals: str, markers: Tuple[str, ...]) -> bool:  # 在依赖推断阶段对高噪声依赖做更保守的命中判断。
    ...
    if dependency_key == 'catch-exception':  # 只有真正出现 catch-exception 的裸 helper 调用时才补这个依赖。
        return 'com.googlecode.catchexception' in normalized_signals or _looks_like_catch_exception_when_usage(normalized_signals)
...
def fix_unreported_exception_declaration(test_file: str, method_name: str, build_output: str) -> Tuple[bool, str]:  # 根据编译日志中的 checked exception 提示补齐目标方法的 throws 声明。
    ...
    if error_line_number is not None:  # 构建日志给出具体报错行时，再尝试按实际报错位置回推所在方法。
        ...
        line_based_method_name = _extract_declared_method_name(lines, line_based_method_start) if line_based_method_start is not None else ''  # 读取按行号定位出的真实方法名，避免把异常加到相邻方法上。
        if method_start is None and line_based_method_start is not None:  # 请求方法本身找不到时再回退到按行号定位的方法。
            method_start = line_based_method_start  # 这样可以覆盖方法名解析失准的场景。
        elif line_based_method_start is not None and line_based_method_name == method_name:  # 只有行号定位和请求方法名一致时才覆盖，避免因编译器行号漂移误修相邻测试方法。
            method_start = line_based_method_start
        elif line_based_method_start is not None and method_start is not None:  # 当前存在两个不同的方法候选时，需要判断行号是否只是轻微漂移到了相邻方法上。
            requested_method_end = _find_method_end(lines, method_start)  # 先读取请求方法自身的结束位置。
            if requested_method_end is None or error_line_number - 1 > requested_method_end + 2:  # 只有报错行明显落在请求方法之后，才接受另一个方法作为真正目标。
                method_start = line_based_method_start
    ...
    actual_method_name = _extract_declared_method_name(lines, method_start) or method_name  # 返回真实被修改的方法名，便于后续诊断。
    return True, f"Declared checked exception on {actual_method_name}: {exception_fqcn}"
...
def fix_missing_imports(test_file: str, build_output: str) -> Tuple[bool, str]:  # 根据编译错误信息为 Java 测试文件自动补充缺失 import 与高置信度符号修复。
    ...
    for method_name in missing_methods:  # 逐个处理缺失的方法名。
        ...
        if qualification_msg:  # 当前缺失方法已经通过限定符修复时无需继续追加 static import。
            ...
            continue  # 进入下一个缺失方法。
        import_path = _resolve_missing_method_reference(test_file, method_name, updated_content)  # 优先利用当前文件和仓库里已有 static import 线索，再结合当前源码语义决定最可信的方法来源。
...
def _resolve_missing_symbol_reference(test_file: str, symbol: str) -> Optional[Tuple[str, Optional[str]]]:  # 为缺失符号解析安全的实际类名与 import 路径。
    ...
    contextual_reference = _resolve_contextual_symbol_reference(test_file, symbol)  # 歧义符号优先结合当前文件上下文判断，避免被仓库里不相干的同名类带偏。
...
def _resolve_missing_method_reference(test_file: str, method_name: str, code_fragment: str = '') -> Optional[str]:  # 为缺失静态方法解析安全的 static import 路径。
    repo_root = _find_repo_root(test_file)  # 先定位当前测试文件所属仓库根目录以便复用仓库内现有 static import 线索。
    contextual_import = _resolve_contextual_method_reference(test_file, method_name, code_fragment)  # `when(...)` 一类方法必须先结合当前补丁正文区分真实断言库。
    if contextual_import:  # 当前文件和补丁正文已经足够说明真正的静态方法来源时直接采用。
        return contextual_import
...
def _resolve_contextual_method_reference(test_file: str, method_name: str, extra_content: str = '') -> Optional[str]:  # 在少量高歧义静态 helper 上结合当前文件和补丁片段做保守判定。
    content = _read_file_quietly(test_file)  # 先读取当前测试文件内容。
    if extra_content:  # generated_patch 或 helper 片段本身往往比当前文件旧 import 更能说明真实语义。
        content = content + '\n' + extra_content
    if method_name == 'when':  # `when(...)` 在 Mockito 与 catch-exception 之间歧义最高。
        if _looks_like_mockito_when_usage(content):  # Mockito stub 场景优先绑定到 Mockito。
            return 'org.mockito.Mockito.when'
        if _looks_like_catch_exception_when_usage(content):  # 只有真正的 catch-exception 语义才绑定到 CatchException.when。
            return 'com.googlecode.catchexception.CatchException.when'
    if method_name == 'caughtException' and _has_unqualified_helper_call(content, 'caughtException'):  # `caughtException()` 只属于 catch-exception。
        return 'com.googlecode.catchexception.CatchException.caughtException'
    return None  # 其余方法继续交给仓库既有线索和静态映射处理。
...
def _resolve_contextual_symbol_reference(test_file: str, symbol: str, extra_content: str = '') -> Optional[Tuple[str, Optional[str]]]:  # 在少量高歧义符号上结合当前文件上下文和当前补丁片段做保守推断。
    ...
    if symbol == 'JsonPath' and any(marker in normalized_content for marker in ('documentcontext', 'readcontext', 'jsonpath.parse(', 'com.jayway.jsonpath')):  # `JsonPath.parse(...)`、`ReadContext` 与 `DocumentContext` 都明显属于 jayway json-path。
        return symbol, 'com.jayway.jsonpath.JsonPath'  # 返回 jayway JsonPath 导入，避免被仓库内的 `JSONPath` 大小写近似类带偏。
    if symbol == 'CollectionUtils':  # `CollectionUtils` 在项目自带工具类与 commons-collections4 之间歧义很高。
        commons_markers = ('isequalcollection(', 'union(', 'intersection(', 'subtract(', 'org.apache.commons.collections4')  # 这些 API 都明显来自 commons-collections4。
        if any(marker in normalized_content for marker in commons_markers):  # 只有正文明确使用 commons-collections4 的 API 时才绑定到第三方依赖。
            return symbol, 'org.apache.commons.collections4.CollectionUtils'  # 返回 commons-collections4 的安全导入路径。
...
def _extract_declared_method_name(lines: List[str], line_idx: Optional[int]) -> str:  # 从方法声明起始行提取真正的方法名，供行号修复和诊断复用。
    if line_idx is None or line_idx < 0 or line_idx >= len(lines):  # 非法行号时直接返回空串。
        return ''
    header_lines = []  # 方法声明可能跨多行，这里把声明头拼起来再统一提取方法名。
    for current_idx in range(line_idx, min(len(lines), line_idx + 4)):  # 方法声明一般不会跨太多行，限制窗口避免吸入方法体。
        header_lines.append(lines[current_idx].strip())  # 去掉缩进便于后续统一正则匹配。
        if '{' in lines[current_idx] or ')' in lines[current_idx]:  # 命中方法体起点或参数列表结束时即可停止。
            break
    header = ' '.join(header_lines)  # 将多行方法声明头拼成单行。
    match = re.search(r'([A-Za-z_][A-Za-z0-9_]*)\s*\(', header)  # 提取最靠近参数列表的标识符。
    return match.group(1) if match else ''  # 找到方法名时返回，否则返回空串。
