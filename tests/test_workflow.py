...
def _make_target(fixed_sha: str = '') -> TestTarget:  # 构造最小可用的测试目标对象。 
    return TestTarget(index=0, repo_url='https://example.com/repo.git', repo_owner='example', project_name='demo', original_sha='a' * 40, fixed_sha=fixed_sha, module='.', full_test_name='com.example.ExampleTest.testCase', input_source='cli')  # 返回单模块最小测试目标。 
...
class WorkflowBehaviorTests(unittest.TestCase):  # 测试工作流编排层的关键行为。 
    ...
    def test_process_request_can_retry_with_fixed_sha_helpers(self):  # 验证 generated_patch 自身上下文不足时，还会尝试把 fixed_sha 中新增的测试 helper 迁回当前工作区。
        request = RunRequest(target=_make_target(fixed_sha='b' * 40), workflow=WorkflowKind.VERIFY_PATCH, runner_backend=RunnerBackend.STANDARD, patch=PatchSpec(generated_patch='public void testCase() {}'))  # 构造一个带 fixed_sha 的最小补丁验证请求。
        with tempfile.TemporaryDirectory() as tmp_dir:  # 创建隔离的工作区目录。
            execution_summary = RerunExecutionSummary(results=['pass', 'pass'], rerun_elapsed_seconds=0.2, checkpoint_rerun_elapsed_seconds={2: 0.2})  # 构造最小执行摘要。
            with mock.patch('rerun_tool.workflow.clone_repo', return_value=_clone_success_result()), mock.patch('rerun_tool.workflow.find_test_file', return_value=str(Path(tmp_dir) / 'ExampleTest.java')), mock.patch('rerun_tool.workflow.apply_patch', return_value=(True, 'OK')), mock.patch('rerun_tool.workflow.build_project', side_effect=[(False, 'cannot find symbol\nsymbol: method assertJsonEqualsNonStrict()'), (True, 'compiled with fixed_sha helper')]) as mocked_build, mock.patch('rerun_tool.workflow.fix_unreported_exception_declaration', return_value=(False, 'no checked exception fix')), mock.patch('rerun_tool.workflow.fix_missing_imports', return_value=(False, 'no safe fix')), mock.patch('rerun_tool.workflow.restore_backup', return_value=False), mock.patch('rerun_tool.workflow.apply_generated_patch_context', return_value=(False, 'context inference failed')), mock.patch('rerun_tool.workflow.backport_fixed_sha_test_helpers', return_value=(True, 'Backported fixed_sha helpers: assertJsonEqualsNonStrict<-fixed_sha:src/test/java/com/example/ExampleTest.java')) as mocked_backport, mock.patch('rerun_tool.workflow.run_test_with_summary', return_value=execution_summary), mock.patch('rerun_tool.workflow.reset_repo', return_value=True):  # 拦截外部副作用并模拟“generated_patch 自身上下文不足，但 fixed_sha helper 能救回”的场景。
                result = process_request(request=request, workspace_dir=tmp_dir, config=_make_config())  # 执行补丁验证工作流。
        self.assertEqual(result.status, 'completed')  # 断言 fixed_sha helper 回溯成功后整个流程仍能正常完成。
        mocked_backport.assert_called_once()  # 断言当前流程确实进入了 fixed_sha helper 回溯阶段。
        self.assertEqual(mocked_build.call_count, 2)  # 断言 helper 回溯后会触发第二次构建。

    def test_process_request_can_retry_with_related_test_import_repairs(self):  # 验证模块内非目标测试文件缺 import 时，工作流也会进入 related test 修复阶段。
        request = RunRequest(target=_make_target(), workflow=WorkflowKind.VERIFY_PATCH, runner_backend=RunnerBackend.STANDARD, patch=PatchSpec(generated_patch='public void testCase() {}'))  # 构造最小补丁验证请求。
        with tempfile.TemporaryDirectory() as tmp_dir:  # 创建隔离的工作区目录。
            execution_summary = RerunExecutionSummary(results=['pass', 'pass'], rerun_elapsed_seconds=0.2, checkpoint_rerun_elapsed_seconds={2: 0.2})  # 构造最小执行摘要。
            with mock.patch('rerun_tool.workflow.clone_repo', return_value=_clone_success_result()), mock.patch('rerun_tool.workflow.find_test_file', return_value=str(Path(tmp_dir) / 'ExampleTest.java')), mock.patch('rerun_tool.workflow.apply_patch', return_value=(True, 'OK')), mock.patch('rerun_tool.workflow.build_project', side_effect=[(False, 'cannot find symbol'), (False, 'other test still missing import'), (True, 'compiled with related test imports')]) as mocked_build, mock.patch('rerun_tool.workflow.fix_unreported_exception_declaration', return_value=(False, 'no checked exception fix')), mock.patch('rerun_tool.workflow.fix_missing_imports', return_value=(False, 'no safe fix')), mock.patch('rerun_tool.workflow.restore_backup', return_value=False), mock.patch('rerun_tool.workflow.apply_generated_patch_context', return_value=(False, 'context inference failed')), mock.patch('rerun_tool.workflow.fix_related_test_imports', return_value=(True, 'src/test/java/com/example/OtherTest.java: Added imports: import org.apache.seatunnel.shade.com.typesafe.config.ConfigFactory;')) as mocked_related_fix, mock.patch('rerun_tool.workflow.backport_fixed_sha_test_helpers', return_value=(False, 'No reusable fixed_sha context found')), mock.patch('rerun_tool.workflow.run_test_with_summary', return_value=execution_summary), mock.patch('rerun_tool.workflow.reset_repo', return_value=True):  # 拦截外部副作用并模拟“目标测试已无安全修复，但同模块其他测试补 import 后可编译”的路径。
                result = process_request(request=request, workspace_dir=tmp_dir, config=_make_config())  # 执行补丁验证工作流。
        self.assertEqual(result.status, 'completed')  # 断言 related test 修复成功后整个流程仍能正常完成。
        self.assertGreaterEqual(mocked_related_fix.call_count, 1)  # 断言当前流程确实进入了 related test import 修复阶段。
        self.assertEqual(mocked_build.call_count, 3)  # 断言 related test 修复会触发额外的一次重新构建。
