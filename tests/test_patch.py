...
class PatchBehaviorTests(unittest.TestCase):  # 测试补丁定位与保护逻辑。
    ...
    def test_find_reference_patch_candidates_ignore_dataset_fixed_code_at_runtime(self):  # 验证即使做离线候选分析，也不会把 fixed_code 或其他数据集代码当成可回放补丁。
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:  # 创建隔离目录存放临时参考源。
            ...
            dataset_csv = Path(tmp_dir) / 'dataset.csv'  # 构造临时原始数据 CSV。
            dataset_csv.write_text(  # 写入一个带 fixed_code 的最小数据集，验证其不会进入运行时候选。
                'project_name,original_sha,module,full_test_name,fixed_code,generated_patch,isCorrect\n'
                f'{entry.project_name},{entry.original_sha},{entry.module},{entry.full_test_name},"public void flakyCase() {{\\n    Assert.assertEquals(1, 1);\\n}}","public void flakyCase() {{\\n    assertEquals(2, 2);\\n}}",1\n',
                encoding='utf-8',
            )  # 完成数据集写入。
            ...
            candidates = find_reference_patch_candidates(entry, reference_root=str(reference_root), dataset_csv=str(dataset_csv))  # 执行候选检索。
        self.assertEqual(len(candidates), 1)  # 断言运行时只会返回参考补丁库中的候选。
        self.assertIn('GoodPatches', candidates[0].source_path)  # 断言返回的候选来自参考补丁库，而不是数据集里的 fixed_code。
