...
class DataLoadingTests(unittest.TestCase):  # 测试统一请求模型与输入加载层。 
    def test_load_patch_requests_wraps_patch_csv(self):  # 验证补丁 CSV 会被正确包装为统一运行请求。 
        csv_content = (  # 构造一个最小可用的补丁数据集。 
            'repo_url,repo_owner,project_name,original_sha,fixed_sha,module,full_test_name,pr_link,flaky_code,fixed_code,diff,generated_patch,isCorrect,source_file,rerun_consistency\n'  # 写入补丁 CSV 表头。 
            'https://github.com/example/demo.git,example,demo,aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa,bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb,.,com.example.ExampleTest.testCase,,old code,,diff text,"public void testCase() {}",1,,CONSISTENT\n'  # 写入单条补丁样本。 
        )  # 完成测试 CSV 内容构造。 
        ...
        self.assertEqual(requests[0].original_rerun_consistency, 'CONSISTENT')  # 断言原始 rerun_consistency 字段会被带入统一请求对象。 
